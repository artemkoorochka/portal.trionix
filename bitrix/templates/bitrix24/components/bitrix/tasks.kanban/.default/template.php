...
foreach ($data["items"] as $k=>$item){
    if(!empty($arResult['DEPENDENCY'][$item["id"]])){
        $data["items"][$k]["data"]["name"] .= '<div class="tasks-kanban-item-status tasks-kanban-item-gray">'
            . Loc::getMessage("KANBAN_PARENT_TITLE")
            . " "
            . $arResult['DEPENDENCY'][$item["id"]]["PARENT_TITLE"]
            . '</div>';
    }
}
