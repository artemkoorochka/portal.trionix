 ...
 * Запретить просмотр звонков сотрудникам из разных отделов в ИСТОРИИ
 * Создан звонок
 * Изменён статус звонка
 * EVENT_TYPE = 1
...
{
    foreach ($arParams['~ROWS'] as $id=>$row){
        /**
         * может видеть свои
         */
        if($row["data"]["CREATED_BY_ID"] == $USER->GetID()){
            continue;
        }
        /**
         * может видеть колег из департамента
         */
        $departaments = Bitrix\Main\UserTable::getList(array(
            "filter" => array(
                "ID" => $USER->GetID()
            ),
            "select" => array(
