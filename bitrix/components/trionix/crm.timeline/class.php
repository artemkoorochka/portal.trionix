...
class CCrmTimelineComponent extends CBitrixComponent
{
	...
	public function prepareScheduleItems()
	{
		...
		while($fields = $dbResult->Fetch())
		{
            ...
            {
                // check by author
                if($items[$fields['ID']]["AUTHOR_ID"] <> $this->userPermissions->GetUserID()){
                    continue;
                }

                // может видеть колег из департамента
                $departaments = Main\UserTable::getList(array(
                    "filter" => array(
                ...
                if(!empty($departaments))
                {
                    ...
                    $users = Main\UserTable::getList(array(
                        "select" => array("ID"),
                    ...
                    if(!in_array($items[$fields['ID']]["AUTHOR_ID"], $departamentUsers)){
                        unset($items[$fields['ID']]);
                    }
                }
            }
		}
	...
	public function validateByDepartment(){
        global $USER;
	    foreach ($this->arResult['HISTORY_ITEMS'] as $id=>$arItem){

	        /**
             * может видеть свои
             */
            if($USER->GetID() == $arItem["AUTHOR_ID"]){
                continue;
            }
            ...
            if(!empty($departaments))
            {
                ...
                if(in_array($arItem["AUTHOR_ID"], $departamentUsers)){
                    continue;
                }
            }
             ...
             */
            unset($this->arResult['HISTORY_ITEMS'][$id]);
        }

        //unset($this->arResult['HISTORY_ITEMS']);
        //d($this->arResult['HISTORY_ITEMS']);
